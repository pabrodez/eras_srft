...
```
```{r table 2}
knitr::kable(subset(df_colorectal[sample(nrow(df_colorectal), 10),], select = -c(patient_number, id)),
...
## Readmissions and mortality (baseline dataset)
...
select(df_colorectal, readm_30, mort_90, mort_30, discharge_date) %>%
  ...
  ## some entries got 1 in 30 day mortality, but 0 in 90 day mortality. 90 day mortality        not capture mortality within 30 days
...
```{r surv facet}
los_km <- survfit(Surv(time = time_event, event = discharge) ~ 1, data = df_surv, type = "kaplan-meier")

ggsurvplot_group_by(los_km,
  data = df_surv,
  group.by = "year_adm",
  surv.median.line = "hv",
  linetype = 1,
