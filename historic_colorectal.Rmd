...
colorectal_merged %>% 
  ...
  ungroup() %>% 
    ggplot(.) +
      geom_histogram(aes(x = los, fill = baseline), binwidth = 1) +
      scale_x_continuous(limits = c(0, 50), breaks = seq(1, 50, 5)) +
      geom_vline(aes(xintercept = median), linetype = 2) +
      geom_text(aes(x = median, label = paste0("Median: ", median), y = 20, hjust = -1)) +
      theme_bw() +
      facet_wrap(~year_adm, ncol = 3)
