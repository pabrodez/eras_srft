...
This document is structured step-wise: From data import to visualization, via data cleaning. Most steps are shown along the chunk of code from the whole script. When necessary code is omitted to avoid repetition.

The data processing of the eras dataset is not documented here because of its length. The full script is [here](https://github.com/pabrodez/eras_srft/blob/master/oeso_eras.Rmd).

The underlying aim is to enhance transparency, which subsequently eases detection of mistakes and improvement.
...
As mentioned above `pulm_supp_24` and `pulm_supp_7` measure similar items but in at different times. `pulm_supp_24` can take values: `r unique(eras_oeso$pulm_supp_24)`. Whereas, `pulm_supp_7`: `r unique(eras_oeso$pulm_supp_7)`. 
\newline
...
Here, confirmed pneumonias and/or ventilation are considered PPCs. 
...
oeso_merged %>%
  ...
  group_by(baseline, week_day, domain) %>%
  ...
  facet_wrap(vars(baseline), labeller = label_both) +
  theme(legend.position = "bottom",
        legend.key.width = unit(1.5, "cm"))
  
```

### It can be interesting to look at rate of iCough compliance by the oesophagectomy patients

```{r echo=FALSE}
eras_compl <- df_srft %>%
  filter(grepl("oesophagectomy", surgery) & (is.na(eras) | eras == "yes")) %>%
  select(inc_spiro, t_brushes, m_washes, oral_diet, mobilised, month_surgery) %>%
  # currently compliant means doing 4 out of 5
  mutate(nas_columns = rowSums(is.na(dplyr::select(., 1:5)))) %>% 
  # how many of the 5 variables got missing values
  mutate(inc_spiro = if_else(inc_spiro %in% c(">two", "once", "twice"), 1, 0, NA_real_)) %>%
  mutate_at(vars(t_brushes, m_washes), function(x) {
    if_else(x == "twice", 1, 0, NA_real_)
  }) %>%
  mutate_at(vars(oral_diet, mobilised), function(x) {
    if_else(x == "yes", 1, 0, NA_real_)
  }) %>%
  mutate(
    compliance_n = rowSums(dplyr::select(., 1:5), na.rm = TRUE),
    # how many items patient is compliant with
    compliant = if_else(compliance_n >= 4, "yes", "no")
  ) 
```

`r tint::margin_note("eras_compl contains all oesophagectomy patients from ERAS dataset and gathers variables relevant to iCough")`
```{r fig.fullwidth=TRUE, fig.width=11}
colours_lines <- c("Rate\nPPC" = "#f4a941", "Rate\ncompliance" = "#4192f4" )
eras_compl %>%
  group_by(month_surgery) %>%
  summarise(
    pts = n(),
    compl_n = sum(compliant == "yes", na.rm = TRUE),
    compl_rate = compl_n / pts
  ) %>%
  mutate(mediana = median(compl_rate)) %>%
  {
    ggplot(., aes(x = month_surgery, y = compl_rate)) +
      geom_line(aes(group = 1, colour = "Rate\ncompliance")) +
      geom_text(aes(label = pts), nudge_y = 0.04) +
      scale_y_continuous(limits = c(0, 1)) +
      annotate("text", x = "2018-08", y = 0.80, label = "IS") +
      annotate("segment",
        x = "2018-08", xend = "2018-08", y = 0.77, yend = 0.67,
        arrow = arrow(length = unit(0.03, "npc"))
      ) +
      theme(
        axis.text.x = element_text(angle = 45, hjust = 1),
      ...
      geom_line(data = filter(merged_ppc, baseline == "no"), 
                aes(x = year_month, y = ppc_rate, group = 1, colour = "Rate\nPPC")) +
      scale_colour_manual(values = colours_lines,
