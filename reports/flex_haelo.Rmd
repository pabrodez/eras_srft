...
# print_measure() takes data, a function that formats it, a time to group by (months or weeks) and area of surgery
# plot_measure() can take data input from print_measure() output and returns a plot
print_measure(df_haelo, measure_fun = count_readm, time=discharge_my)  ## time = week_start or discharge_my
plot_measure <- function(data, x = week_start, y = y, measure_fun) {
  x_aes <- enquo(x)
  y_aes <- enquo(y)
  {ggplot(data = data, aes(x = !! x_aes, y = !! y_aes, text = sprintf("Patients: %s\n%s", n_patients, !! x_aes))) +
...
print_measure(df_haelo, measure_fun=count_surgery, time=week_start) %>% 
  plot_measure(data = ., x = week_start, y = y)
```
...
print_measure(df_haelo, measure_fun=count_readm, time=discharge_my) %>% 
  plot_measure(data = ., x = discharge_my, y = y)
```
