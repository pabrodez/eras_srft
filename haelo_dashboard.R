...
colorectal_merged <- df_srft %>%
  filter(area_surgery == "colorectal" & eras == "yes") %>%  ## eras and colorectal pts
...
colorectal_merged <- colorectal_merged %>% mutate(
  ...
  month_adm =  as.Date(format.Date(admission_date, "%Y-%m")),
...
colorectal_merged %>%
  ...
  {
    ggplot(.) +
      geom_line(aes(x = month_dis, y = median, group = 1, colour = baseline), linetype = 5) +
      geom_line(aes(x = month_dis, y = mean, group = 1, colour = baseline), linetype = 1)
