...
## Purpose 2: Format baseline colorectal data
###################################################################################
...
get_school <- function(data) {
        data %>%
                dplyr::select(area_surgery, surgery_school, chest_physio, week_start) %>%
                # filter(area_surgery == "")  ## can be used here to select a specific area of surgery
                group_by(week_start) %>%
                summarise(
                        n = n(),
                        school_n = sum(
...
df_haelo %>%
  ...
  ) %>% ## number of patients with NA in tbrushes column
  print.data.frame()
...
# 1.4.12 Function to conveniently print any of the above by area of surgery
print_measure <- function(data, measure_fun, area = "all") {
  if (area == "all") {
    measure_fun(data)
  } else {
...
ggplot(discharge_df) +
  geom_line(
    data = subset(melt(discharge_df, id = "month"), variable %in% c("median", "mean")),
    aes(x = month, y = value, colour = variable, group = variable)
  ) +
  # geom_text(
  #   position = position_dodge(.9),
  #   aes(x = month, y = mean, label = count)
  # ) +
  theme_fivethirtyeight() +
