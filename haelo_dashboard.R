...
## Purpose 1.2: Read oesophagectomy data and merge with oesophagectomy patients in eras dataset
...
double_surg <- df_colorectal %>%  ## Index of entries with same patient undergoing two surgeries on same day
  ...
  {.[duplicated(.), "patient_number"]} %>% 
  pull(1) %>%
  {which(df_colorectal$patient_number %in% .)}
