...
df_oeso <- read.csv("./data/ppt_oesophagectomies.csv",
  ...
  na.strings = c("n/a", "NA"),
  col.names = c("surgery_date", "pneumonia", "pulm_supp_24", "pulm_supp_7", "abx", "notes")
)
```
...
df_oeso %>%
  ggplot(.) +
  geom_bar(aes(x = pulm_supp_24)) +
  scale_y_continuous(breaks= pretty_breaks())
  
```
```{r oxy_table, echo=FALSE}
table(Oxygen = df_oeso$pulm_supp_24, useNA = "ifany") %>% 
  kable(.) 
```
...
df_oeso %>%
  ggplot(.) +
  geom_bar(aes(x = pulm_supp_7))
  
```
```{r vent_table, echo=FALSE}
table(ventilated = df_oeso$pulm_supp_7, useNA = "ifany") %>% 
  kable(.) 
```
...
## Preparation ERAS dataset before merge
...
```{r eras_oeso_columns}
eras_oeso <- eras_oeso %>%
  select(surgery_date, infec_7, pulm_supp_24, pulm_supp_7, infec_24)
```
...
```{r}
eras_oeso <- eras_oeso %>% 
  mutate(pneumonia = if_else(infec_7 == "chest", "yes", "no", NA_character_))
```
`r tint::margin_note("If patient had new requirement for oxygen or respiratory support within 24h before day 7, substitute by yes, else no")`
```{r}
eras_oeso <- eras_oeso %>% 
  mutate(pulm_supp_24 = if_else(pulm_supp_24 %in% c("new requirement for oxygen", "new requirement for respiratory support"), "yes", "no", NA_character_))
```

`r tint::margin_note("If patient had new requirement for respiratory support within 7 days, substitute by yes, else no")`
```{r}
eras_oeso <- eras_oeso %>% 
  mutate(pulm_supp_7 = if_else(pulm_supp_7 %in% c("moderate", "severe"), "yes", "no", NA_character_))
```
