...
\newline
This document is structured step-wise: From data import to visualization, via data cleaning. Most steps are shown along the chunk of code from the whole script.
\newline
...
df_oeso <- read.csv("./data/ppt_oesophagectomies.csv",
  ...
  na.strings = c("n/a"),
  col.names = c("op_date", "pneumonia", "oxigen", "ventilated", "abx", "notes")
)

```

```{r date and yes/no, echo=TRUE}
df_oeso$op_date <- as.Date(df_oeso$op_date, "%d/%m/%Y")

df_oeso <- df_oeso %>% mutate(
  pneumonia = if_else(pneumonia == "1", "yes", "no"),
  oxigen = if_else(oxigen == "1", "yes", "no"),
  ventilated = if_else(ventilated == "1", "yes", "no"),
  abx = if_else(abx == "1", "yes", "no")
)
```


```{r}
df_oeso %>% select(pneumonia, oxigen, ventilated, abx) %>% 
  map_dfc(~ mutate(.x, if_else(.x == "1", "yes", "no")))
```
