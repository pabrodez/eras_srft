...
df_srft <- read.csv(data_path, strip.white = TRUE, stringsAsFactors = FALSE, nrows = 210)
...
# Plots LOS ---------------------------------------------------------------
# Avg LOS per month of D/C
df_srft %>%
  select(discharge_date, hospital_stay) %>%
  transmute(stay = hospital_stay, dc_month = lubridate::month(discharge_date, label = TRUE)) %>%
  na.omit() %>%
  group_by(dc_month) %>%
  summarise(avg_los = mean(stay, na.rm = TRUE)) %>%
  ggplot(data = ., aes(x = dc_month, y = avg_los)) +
  geom_col() +
  theme_fivethirtyeight()
...
df_srft %>% select(inc_spiro, m_washes, t_brushes, oral_diet, mobilised, hospital_stay) %>% 
  ...
  na.omit() %>% 
  mutate(compl_group = rowSums(select(., 1:5))) %>%
...
df_srft %>% select(inc_spiro, m_washes, t_brushes, oral_diet, mobilised, hospital_stay, area_surgery) %>% 
  mutate(inc_spiro = if_else(inc_spiro %in% bundle_yes, 1, 0),
         m_washes = if_else(m_washes == "twice", 1, 0),
         t_brushes = if_else(t_brushes == "twice", 1, 0),
         oral_diet = if_else(oral_diet == "yes", 1, 0),
...
df_srft %>%
  ...
  mutate(morbid = rowSums(select(., 1:9))) %>%
...
measures_plots <- c(measures_plots, list(
  df_srft %>%
    ...
    labs(
      title = "attended surgery school by week"
    ) +
    theme_fivethirtyeight() +
    theme(axis.text.x = element_text(angle = 45, hjust = 1))
))
# TO DO: Proportion of colorectal pts. with PPC within 7 days
# Proportion of pts. compliant with icough bundle
measures_plots <- c(measures_plots, list(
  df_srft %>%
    select(inc_spiro, t_brushes, m_washes, oral_diet, mobilised, week_start) %>%
    gather(value = "value", key = "key", -week_start) %>%
    transmute(week_start = week_start, compliant = if_else(value %in% bundle_yes, 1, 0)) %>%
    group_by(week_start) %>%
    summarise(prop = sum(compliant) / n()) %>%
    ...
    scale_y_continuous(limits = c(0, 1)) +
    labs(
      title = "complied with icough bundle"
    ) +
    theme_fivethirtyeight() +
    theme(axis.text.x = element_text(angle = 45, hjust = 1))
))
...
measures_plots <- c(measures_plots, list(
  df_srft %>%
    ...
    scale_y_continuous(limits = c(0, 1)) +
    labs(
      title = "brushed teeth twice\nwithin 1 day after surg."
    ) +
    theme_fivethirtyeight() +
    theme(axis.text.x = element_text(angle = 45, hjust = 1))
))
...
measures_plots <- c(measures_plots, list(
  df_srft %>%
    select(mobilised, week_start) %>%
    na.omit() %>%
    mutate(mobilised = if_else(mobilised == "yes", 1, 0)) %>%
    group_by(week_start) %>%
    summarise(freq = sum(mobilised) / n()) %>%
    ggplot(data = ., aes(x = as.Date(week_start), y = freq)) +
    geom_line(colour = "#FF2700", group = 1) +
    geom_point(size = 1) +
    scale_x_date(date_breaks = "1 week", date_labels = "%b-%d") +
...
df_suv <- df_srft %>%
  select(surgery_date, discharge_date, 
  ...
  mutate(poms = if_else(rowSums(select(., 3:11)) == 0, "no", "yes"))
...
ggsurvplot_combine(list(poms = los_km_poms, general = los_km), 
                   ...
                   break.x.by = 3,
                   ggtheme = theme_fivethirtyeight())
# Combine 2 general curves: (i) consider all patients and those d/c before 7 days are considered not to have morb
