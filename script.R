...
df_srft %>% 
        ...
        labs(title = "Prop. pts. who attended surgery school by week",
             subtitle = paste("From ", min(as.Date(df_srft$week_start)), "to ", max(as.Date(df_srft$week_start)))) +
        theme_fivethirtyeight() +
...
df_srft %>%
  ...
  scale_y_continuous(limits = c(0, 1)) +
labs(
  title = "Proportion of pts. compliant with icough bundle",
  subtitle = paste("From ", min(as.Date(df_srft$week_start)), "to ", max(as.Date(df_srft$week_start)))
) +
  theme_fivethirtyeight() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
...
df_srft %>% select(t_brushes, week_start) %>% na.omit() %>% 
        mutate(t_brushed = if_else(t_brushes == "twice", 1, 0)) %>% 
        group_by(week_start) %>% summarise(freq = sum(t_brushed)/n()) %>% 
        ...
        scale_y_continuous(limits = c(0, 1)) +
        labs(
                title = "Proportion of pts. who brushed teeth twice\nwithin 1 day after surg.",
                subtitle = paste("From ", min(as.Date(df_srft$week_start)), "to ", max(as.Date(df_srft$week_start)))
        ) +
        theme_fivethirtyeight() +
        theme(axis.text.x = element_text(angle = 45, hjust = 1))
...
# Prop pts mobilised
df_srft %>% select(mobilised, week_start) %>% na.omit() %>% 
        mutate(mobilised = if_else(mobilised == "yes", 1, 0)) %>% 
        group_by(week_start) %>% summarise(freq = sum(mobilised)/n()) %>% 
        ggplot(data =., aes(x = as.Date(week_start), y = freq)) +
        geom_line(colour = "#FF2700", group = 1) +
        geom_point(size = 1) +
        scale_x_date(date_breaks = "1 week", date_labels = "%b-%d") +
        scale_y_continuous(limits = c(0, 1)) +
        labs(
                title = "Proportion of pts. mobilised\nwithin 1 day after surg.",
                subtitle = paste("From ", min(as.Date(df_srft$week_start)), "to ", max(as.Date(df_srft$week_start)))
        ) +
        theme_fivethirtyeight() +
        theme(axis.text.x = element_text(angle = 45, hjust = 1))
# Prop pts used IS
df_srft %>% select(inc_spiro, week_start) %>% na.omit() %>% 
        mutate(inc_spiro = if_else(inc_spiro %in% c("once", "twice", ">two"), 1, 0)) %>% 
        group_by(week_start) %>% summarise(freq = sum(inc_spiro)/n()) %>% 
        ggplot(data =., aes(x = as.Date(week_start), y = freq)) +
        geom_line(colour = "#FF2700", group = 1) +
        geom_point(size = 1) +
        scale_x_date(date_breaks = "1 week", date_labels = "%b-%d") +
        scale_y_continuous(limits = c(0, 1)) +
        labs(
                title = "Proportion of pts. who used IS\nwithin 1 day after surg.",
                subtitle = paste("From ", min(as.Date(df_srft$week_start)), "to ", max(as.Date(df_srft$week_start)))
        ) +
        theme_fivethirtyeight() +
        theme(axis.text.x = element_text(angle = 45, hjust = 1))
# Prop pts mouth washed twice
df_srft %>% select(m_washes, week_start) %>% na.omit() %>% 
        mutate(m_washed = if_else(m_washes == "twice", 1, 0)) %>% 
        group_by(week_start) %>% summarise(freq = sum(m_washed)/n()) %>% 
        ggplot(data =., aes(x = as.Date(week_start), y = freq)) +
        geom_line(colour = "#FF2700", group = 1) +
        geom_point(size = 1) +
        scale_x_date(date_breaks = "1 week", date_labels = "%b-%d") +
        scale_y_continuous(limits = c(0, 1)) +
        labs(
                title = "Proportion of pts. who used mout wash twice\nwithin 1 day after surg.",
                subtitle = paste("From ", min(as.Date(df_srft$week_start)), "to ", max(as.Date(df_srft$week_start)))
        ) +
        theme_fivethirtyeight() +
        theme(axis.text.x = element_text(angle = 45, hjust = 1))
...
# Prop pts oral diet
df_srft %>% select(oral_diet, week_start) %>% na.omit() %>% 
        mutate(oral_diet = if_else(oral_diet == "yes", 1, 0)) %>% 
        group_by(week_start) %>% summarise(freq = sum(oral_diet)/n()) %>% 
        ggplot(data =., aes(x = as.Date(week_start), y = freq)) +
        geom_line(colour = "#FF2700", group = 1) +
        geom_point(size = 1) +
        scale_x_date(date_breaks = "1 week", date_labels = "%b-%d") +
        scale_y_continuous(limits = c(0, 1)) +
        labs(
                title = "Proportion of pts. re-started oral diet\nwithin 1 day after surg.",
                subtitle = paste("From ", min(as.Date(df_srft$week_start)), "to ", max(as.Date(df_srft$week_start)))
        ) +
        theme_fivethirtyeight() +
        theme(axis.text.x = element_text(angle = 45, hjust = 1))
