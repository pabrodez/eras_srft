...
df_srft %>%
        ...
        labs(title = "Patients compliant with iCough bundle by area",
             subtitle = "Compliant with 4 in 5: mobilisation, drinking, diet, mouth wash, teeth brush or IS") +
        theme_fivethirtyeight() +
...
df_srft %>% 
        ...
        group_by(area_surgery) %>% summarise(avg_los = mean(hospital_stay, na.rm = TRUE)) %>% 
        ggplot(data = .) + geom_col(aes(x = area_surgery, y = avg_los)) +
