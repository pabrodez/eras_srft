...
# Day of the week of admission and LOS
df_srft %>% 
        select(surgery_date, 
               hospital_stay) %>% 
...
#     los = if_else(is.na(discharge_date) == TRUE, Sys.Date() - surgery_date, discharge_date - surgery_date)
#   )
# 
# los_km <- survfit(Surv(time = los, event = discharge) ~ 1, data = df_suv) 
# 
