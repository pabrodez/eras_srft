...
# file_url <- "https://raw.githubusercontent.com/pabrodez/eras_data/master/eras_srft_30Nov18.csv"
...
df_srft <- read.csv(data_path, strip.white = TRUE, stringsAsFactors = FALSE, nrows = 250)
...
# what proportion of patients were readmitted grouped by month of surgey?
df_srft %>%
  dplyr::select(surgery_date, readmit_30) %>%
  mutate(dc_month = lubridate::month(surgery_date, label = TRUE)) %>%
  na.omit() %>%
  ...
  geom_col() +
  labs(title = "Rel. freq. R/A grouped by month of surg.",
       fill = "Readmitted?") +
  theme_fivethirtyeight()
...
df_srft %>%
  select(surgery_date, hospital_stay) %>%
  transmute(stay = hospital_stay, surg_month = lubridate::month(surgery_date, label = TRUE)) %>%
  na.omit() %>%
  group_by(surg_month) %>%
  summarise(avg_los = mean(stay, na.rm = TRUE)) %>%
  ggplot(data = ., aes(x = surg_month, y = avg_los)) +
  geom_col() +
  labs(title = "Avg LoS per month of surgery",
