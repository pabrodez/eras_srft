...
# Surgeons count of surgeries
surgeons_df <- transform(table(unlist(strsplit(df_srft$surgeon, ",\\s"))))
surgeons_df$Var1 <- factor(surgeons_df$Var1, levels = surgeons_df$Var1[order(surgeons_df$Freq)])

ggplot(data = surgeons_df, aes(x = Var1, y = Freq)) + 
  geom_col() +
  scale_y_continuous(breaks = seq(min(surgeons_df$Freq), max(surgeons_df$Freq), 2)) +
  theme_fivethirtyeight() +
  coord_flip()

# Are of surgery
area_df <- transform(table(df_srft$area_surgery))
area_df$Var1 <- factor(surgeons_df$Var1, levels = surgeons_df$Var1[order(surgeons_df$Freq)])
